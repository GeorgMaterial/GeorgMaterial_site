...
function findSong(songName){
    for (let item of data.songs.content){
        if (songName == item.name){
            song = item;
        }
    }
    return song;
                ...
                listen:[
                    {
                        ...
                        apple_music_url: 'https://music.apple.com/au/album/glue/1505991360?i=1505991363',
                        youtube_url: 'https://www.youtube.com/watch?v=_w1gmU9B8qg',
                        bandcamp_url: 'https://georgiamarley.bandcamp.com/track/enjoy-the-view'
                    }
                ],
                lyrics: '',
                about:''
        ...
        build: function (){
            ...
            for (let child of songListEl.children){
                child.addEventListener('click', function(event){
                    let link = event.composedPath()[0].textContent;
                    centerContent.replaceChildren(data.songs.songBuild(link));
                })
            }
            return fragment;
        }
